...
use knurled_core::{
    ActualSet, ExecutionInput, ItemInput, SCHEMA_VERSION, ValidationStatus, backtest_records_repo,
    init_training_repo, read_records, read_state, read_training_repo, render_next, submit_repo,
...
fn completing_a_session_clears_an_outstanding_partial() {
    ...
    // Finish the same session fresh on a later date.
    let state = read_state(&root).unwrap();
    let rendered = render_next(&repo.compiled, &state).unwrap();
    assert_eq!(rendered.session_id, session_id, "cursor stayed on the partial session");
    let complete = synthetic_execution_input(&rendered, "pass", 0);
    submit_repo(&root, &complete, SubmitMode::Advance, "2026-06-26").unwrap();
    ...
    // And the cursor advanced past the completed session.
    assert_ne!(read_state(&root).unwrap().cursor.next_session, session_id);
